...
subroutine mixedlayer_restrat_OM4(h, uhtr, vhtr, tv, forces, dt, h_MLD, VarMix, G, GV, US, CS)
  ...
  real, dimension(SZI_(G),SZJ_(G)) :: &
    ...
    lf_f_lat, &           !wgl Frontal width as a specified function of f(Latitude)    [H ~> m or kg m-2]
  ...
  real :: vonKar_x_pi2    ! A scaling constant that is approximately the von Karman constant times
                          ! pi squared [nondim]
  real :: logzero, logcut, logmin, fcut, fmin, I_LF_f, mpwr !f-dependent frontal scale
...
!wgl  G  f-dependent frontal scale parameters
  logzero =  0.0 
  logcut  = -1.0
  logmin  = -1.5
  fcut    =  0.5e-4
  ...
  if (CS%front_length>0.) then
    ...
    do j=js-1,je+1 ; do i=is-1,ie+1
      lf_f_lat(i,j) = CS%front_length
      absf = 0.25*abs(G%CoriolisBu(I,J)+G%CoriolisBu(I,J-1)+G%CoriolisBu(I-1,J)+G%CoriolisBu(I-1,J-1))
      if ( absf < fcut ) then
        mpwr = logzero - absf * (logzero-logcut) /  (fcut) 
      elseif ( absf > fmin ) then
        mpwr = logmin
      else
        mpwr = logcut - (absf-fcut) * (logcut-logmin) / (fmin-fcut)
      endif
      lf_f_lat(i,j) = 1000.*10**(mpwr)
    enddo ; enddo
  else
    res_upscale = .false.
  endif
...
!   U - Component
  ...
  do j=js,je ; do I=is-1,ie
    ...
    absf = 0.5*(abs(G%CoriolisBu(I,J-1)) + abs(G%CoriolisBu(I,J)))               !wgl NOT zero at the equator
    I_LFront = 1. / CS%front_length
!wgl  f-dependent frontal scale
    I_LFront = 2. / ( lf_f_lat(i,j) + lf_f_lat(i+1,j) )
...
!  V- component
  ...
  do J=js-1,je ; do i=is,ie
    ...
    I_LFront = 1. / CS%front_length
!wgl  f-dependent frontal scale
    I_LFront = 2. / ( lf_f_lat(i,j) + lf_f_lat(i,j+1) )
  ...
  if (query_averaging_enabled(CS%diag)) then
    ...
    if (CS%id_lfbod         > 0) call post_data(CS%id_lfbod, lf_f_lat, CS%diag)
